      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          {sections.map((s) => (
            <a
              ...
              className={clsx(
                ...
                active === s.id
                  ? "text-[var(--accent)]"
                  : "text-[var(--muted-text)] hover:text-[var(--accent)]"
              )}
            >
              {s.label}
            </a>
        ...
        <div className="flex items-center gap-2 ml-auto md:ml-0">
          ...
          <a
            href="/api/resume"
            download="resume.pdf"
            className="inline-flex items-center gap-1 px-3 py-2 text-sm rounded-md btn-accent"
          >
            <FileDown size={16} /> Resume
          </a>
          {/* Mobile hamburger (hides links into drawer) */}
          <button
            className="md:hidden inline-flex items-center justify-center w-9 h-9 rounded-md border border-[var(--border)] bg-[var(--surface)]/70"
            onClick={() => setOpen((v) => !v)}
